...
class Psikit(object):
    ...
    def optimize(self, basis_sets= "scf/6-31g**", return_wfn=True, name=None, multiplicity=1, maxiter=50):
        ...
        if not self.debug:
            ...
            #except:
            self.psi4.core.clean_variables()
        return scf_energy
	...
	# fix version issue get_variable is now variable
        #x = self.psi4.get_variable('SCF DIPOLE X')
        #y = self.psi4.get_variable('SCF DIPOLE Y')
        #z = self.psi4.get_variable('SCF DIPOLE Z')
        scfdipole=self.psi4.variable('SCF DIPOLE')
        x=scfdipole[0]
        y=scfdipole[1]
        z=scfdipole[2]
        #x = self.psi4.variable('SCF DIPOLE X')
        #y = self.psi4.variable('SCF DIPOLE Y')
        #z = self.psi4.variable('SCF DIPOLE Z')
